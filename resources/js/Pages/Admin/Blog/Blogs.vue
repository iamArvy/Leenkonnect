<template>
    <AdminLayout>
        ...
        <ul class="flex flex-col gap-2 bg-white rounded-2xl overflow-hidden py-2 text-center">
            ...
            <li v-for="(item, index) in blogs.data" :key="index">
                <ul class="grid grid-cols-4 gap-2 h-[50px]">
                    <li>{{ item.title }}</li>
                    <li>{{ item.date }}</li>
                    <li>
                        <UpdateBlog :post="item" :categories="categories" />
                    </li>
                    <li>
                        <!-- @vue-ignore -->
                        <DangerButton @click="router.delete(route('admin.blog.posts.destroy', item.id))">Delete</DangerButton>
                    </li>
                </ul>
            </li>
        </ul>
        <Pagination v-if="blogs.links.length <= 0" :prev="blogs.prev_page_url" :next="blogs.next_page_url" @paginate="paginate" />
    </AdminLayout>
</template>
...
import CreateBlog from './Partials/CreateBlog.vue';
import UpdateBlog from './Partials/UpdateBlog.vue';
import DangerButton from '@/Components/Admin/DangerButton.vue';
const props = defineProps({blogs: Object, categories: Array, filters: Object})
const filters = reactive({ ...props.filters });
const applyFilters = () => {
    // @ts-ignore
  router.get(route('admin.blog.posts.index'), filters, { preserveState: true });
};
const paginate = (url) => {
